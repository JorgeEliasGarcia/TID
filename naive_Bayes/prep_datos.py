...
def cambios_de_tipo(): 
  ...
  mydata['Dependents'] = mydata['Dependents'].replace('3+', 4) # Asignamos un valor numérico a '3+'. Le asigno un 3, porque lo más probable es que si tienen 3 hijos o más tengan 3 hijos.
...
def identificar_variables_validas_para_agrupacion():
  ...
  # Para poder agrupar por más variables, si una variable solo toma valores nulos 15 veces o menos, eliminamos esas filas.
...
def hacer_agrupacion_clusters(selected_columns):
  ...
  kmeans = KMeans(n_clusters=5)  #  Aplicamos kMeans, usando 5 clusters
...
def diferencia_balance_clases():
  approved_loans = len(mydata[mydata["LoanStatus"] == True])
  not_approved_loans = len(mydata[mydata["LoanStatus"] == False])
...
def division_datos_entrenamiento_prueba(data): 
  X = data.drop("LoanStatus", axis=1)  # Características
  y = data["LoanStatus"]  # Etiquetas
...
def clasificador_KNN():
  ...
  X_train, X_test, y_train, y_test = division_datos_entrenamiento_prueba(x_numeric)  # Dividimos los datos en conjuntos de entrenamiento y prueba. IMPORTANTE, estamos manteniendo la proporción de clases. 
