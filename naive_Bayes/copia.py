...
def detectar_y_eliminar_outliers(data, contamination = 0.09): # Esperamos que aproximadamente el 5% de los datos sean outliers. 
  ...
  if len(outliers_indices) < len(data) * 0.09:   # Eliminamos los outliers detectados si representan menos del 5% de la base de datos
...
pipeline = ColumnTransformer(
    [   
        ("text", OneHotEncoder(), ["Gender", "Education", "PropertyArea", "Married", "SelfEmployed"]),
        ("numeric", StandardScaler(), ["ApplicantIncome", "CoapplicantIncome", "LoanAmount", "LoanAmountTerm", "Dependents"])
    ], 
)
